import React, { useState, useEffect, useRef } from 'react';
      ...
      const ApplicationPage = () => {
        ...
        const [countryCodeWidth, setCountryCodeWidth] = useState(120);
          ...
          fetch('country_codes.txt')
            ...
            .then(data => {
              const codes = data.split('\n').map(line => {
                ...
                const code = parts.shift(); // Extract the code
                const name = parts.join(' '); // Join the remaining parts as the name
                return { code: code?.trim() || '', name: name?.trim() || '' };
              }).filter(country => country.code !== ""); // Filter out empty lines
        ...
        useEffect(() => {
          // Dynamically calculate the width of the select element based on the selected option
          if (selectRef.current) {
            const selectedOption = countryCodes.find(country => country.code === formData.countryCode);
            if (selectedOption) {
              const textWidth = 30 + ((selectedOption.code.length + selectedOption.name.length) * 8);
              setCountryCodeWidth(Math.max(textWidth, 152)); // Base width of 152px
            }
          }
        }, [formData.countryCode, countryCodes]);
                  ...
                  <div>
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex rounded-md shadow-sm">
                          <select
                            ...
                            onChange={handleInputChange}
                            className="relative rounded-l-md border border-gray-200 bg-white focus:border-blue-500 focus:ring-blue-500 sm:text-sm h-11 px-4"
                            style={{ width: `${countryCodeWidth}px`, overflowY: 'scroll', maxHeight: '200px' }}
                            ref={selectRef}
                          >
                          ...
                          <input
                            ...
                            onChange={handleInputChange}
                            className="relative flex-grow min-w-0 w-full rounded-r-md border border-gray-200 bg-white focus:border-blue-500 focus:ring-blue-500 sm:text-sm h-11 px-4"
                    ...
                    <div className="mt-8 flex justify-between">
                      ...
                      <button
                        ...
                        className="ml-auto flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed"
                      >
                        Neste
                        <ChevronRight className="h-5 w-5 ml-1" />
                      </button>
